...
with open('./bin/sc/MerkleTreeForAirDrop.nef', 'rb') as f:
    merkle_tree_nef = f.read()
with open('./bin/sc/MerkleTreeForAirDrop.manifest.json', 'r') as f:
...
client.invokefunction('anyUpdate', params=[merkle_tree_nef, merkle_tree_manifest, 'computeMerkleTreeRoot', [accounts, amounts]], relay=False)
client.print_previous_result()
client.invokefunction('anyUpdate', params=[merkle_tree_nef, merkle_tree_manifest, 'computeMerkleTree', [accounts, amounts]], relay=False)
client.print_previous_result()
tree = client.previous_result
wallet_leaf_hash = client.invokefunction('anyUpdate', params=[merkle_tree_nef, merkle_tree_manifest, 'leafHash', [wallet_hash, 19_999_999_999]], relay=False)
client.print_previous_result()
assert wallet_leaf_hash == tree[2][0]

user1_leaf_hash = client.invokefunction('anyUpdate', params=[merkle_tree_nef, merkle_tree_manifest, 'leafHash', [user1_hash, 18_888_888_888]], relay=False)
client.print_previous_result()
assert user1_leaf_hash == tree[2][1]

user2_leaf_hash = client.invokefunction('anyUpdate', params=[merkle_tree_nef, merkle_tree_manifest, 'leafHash', [user2_hash, 17_777_777_777]], relay=False)
client.print_previous_result()
...
assert tree[0][0] == client.invokefunction(
    ...
    params=[
        ...
        [wallet_hash, 19_999_999_999, [user1_leaf_hash, internal_right_hash]]], relay=False)
